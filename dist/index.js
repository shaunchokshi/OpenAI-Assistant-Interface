...
function logRequest(method, path5, statusCode, duration, responseData) {
  const logData = {
    method,
    path: path5,
    statusCode,
  ...
  logger[level](`${method} ${path5} ${statusCode} in ${duration}ms`, logData);
}
  ...
  const gracefulShutdown = (signal) => {
    ...
    server.close((err) => {
      ...
      try {
        ...
        logger.error(`Error closing database: ${err2.message}`);
        process.exit(1);
      }
    });
  };
